...
main = hspec $ do
    ...
    describe "Day 9" $ do
        it "Can run a quine using relative mode" $
            runMachine quine [] `shouldBe` (109, reverse quine)
        it "Can handle 16-digit numbers multiplied" $
            runMachine [1102,34915192,34915192,7,4,7,99,0] [] `shouldBe` (1102, [1219070632396864])
        it "Can handle 16-digit numbers immediate" $
            runMachine [104,1125899906842624,99] [] `shouldBe` (104, [1125899906842624])
